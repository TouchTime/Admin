<template>
  <div class="self-orders-box">
    ...
    <el-table v-loading="loading" :data="tableData">
      ...
      <el-table-column prop="createTime" label="创建时间" min-width="160" align="center" :formatter="(row, column, cellValue)=>formatTime(cellValue)"></el-table-column>
      ...
      <el-table-column prop="paySum" label="支付总金额" min-width="100" align="center"></el-table-column>
      ...
      <!-- 新增金额 -->
      <el-table-column prop="address" label="余额外支付金额" min-width="120" align="center"></el-table-column>
      <el-table-column prop="finalSum" label="最终金额" min-width="120" align="center"></el-table-column>
      <el-table-column prop="" label="操作" min-width="200" align="center">
        <template slot-scope="scope">
        	<el-button size="mini" @click="handleShow(scope.$index, scope.row)">明细</el-button>
          <el-button v-if="(queryForm.type == 1) && (scope.row.status >= 2 && scope.row.status < 5) " type="primary" size="mini" @click="handleSureSend(scope.$index, scope.row)">确认发货</el-button>
        </template>
      </el-table-column>
    ...
    methods : {
      ...
      queryTotal(param){
        Request({
          ...
          data :Object.assign({},{
            type : 1,
            takeType: (this.queryForm.type == '2') ? '' : this.queryForm.type,
      ...
      exportExcel(){
        ...
        let queryStr = `type=1&takeType=0&`;
        ...
        if(sendData.endTime){
          sendData.endTime = sendData.endTime.split(' ')[0] + ' 23:59:59';
        }
        ...
        window.location.href = `/platforms/platform/order/pickexport?${queryStr}`;
      }
    }
  }
</script>
